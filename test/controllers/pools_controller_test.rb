...
class PoolsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pool" do
    assert_difference('Pool.count') do
      post pools_url, params: { pool: { end_ip: @pool.end_ip, name: @pool.name, start_ip: @pool.start_ip, subnet_id: @pool.subnet_id } }
    end
  ...
  test "should update pool" do
    patch pool_url(@pool), params: { pool: { end_ip: @pool.end_ip, name: @pool.name, start_ip: @pool.start_ip, subnet_id: @pool.subnet_id } }
