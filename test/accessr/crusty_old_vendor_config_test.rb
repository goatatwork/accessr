...
class CrustyOldVendorConfigTest < ActionDispatch::IntegrationTest
  ...
  test "input rate limit should be nil for a port that has no input rate" do
    config = CrustyOldVendorConfig.new(fake_fastiron_config_with_missing_rate_limits)

    assert_nil config.input_rate_limit("1/1/14")
  end

  test "output rate limit should be nil for a port that has no output rate" do
    config = CrustyOldVendorConfig.new(fake_fastiron_config_with_missing_rate_limits)

    assert_nil config.output_rate_limit("1/1/14")
  end
  ...
  def fake_fastiron_config_with_missing_rate_limits
    content = <<~FAKE_CONFIG
      show running-config
